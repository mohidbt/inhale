...
| **2.0.2 — UX polish & bugfixes** | IN PROGRESS (fixes pending — see `docs/superpowers/plans/2026-04-14-phase-2.0.2-fixes.md`) | Tasks 24–36 shipped on `feature/phase-2.0.2-ux-polish`. Highlights render as real rects via `UserHighlightLayer` (new `user_highlights.rects` jsonb + `source` + `layer_id` + `comment`; migration `0003_extend_user_highlights.sql`). `OutlineSidebar` rewritten — Pages tab always, Contents tab from native `pdf.getOutline()`. `FindBar` + `usePdfFind` (pdfjs `PDFFindController`/`EventBus`/`PDFLinkService`) wired to Cmd/Ctrl+F. `SelectionToolbar` gains Comment + Ask AI modes; highlights sidebar shows comments inline; `/highlights/[id]` PATCH for comment edits. Legacy comment UI + `user_comments` table removed (migration `0004_drop_user_comments.sql`). `/api/ai/explain` + `ConceptsPanel` deleted. Pinch / ctrl+wheel zoom on the PDF container (clamped 0.5–3). `DockableSidebar` wraps all right-sidebar panels via `react-resizable-panels`; outline defaults to left dock; dock + width persist to localStorage. Collapsible top toolbar with hover-to-reveal (persisted). Chat history drawer — `GET /api/documents/[id]/conversations` + `GET /api/conversations/[id]/messages`; `useChat` exposes `loadConversation` / `conversationId`; New + History buttons in `ChatPanel`. Ask-AI seeds chat via `{ text, nonce }` prop. **Unit tests:** 155 passing (+ 6 pre-existing fixture-missing failures unrelated). **Playwright suite:** 20/21 of touched specs green; `chat-history.spec.ts` (2/2) covers the drawer; stale "Comments sidebar" + old outline-sections mocked tests pruned. Remaining failure (`Next button advances … on real paper`) is a pre-existing smooth-scroll timing flake, not regressed by this phase. **Chrome DevTools MCP walkthrough** per spec checklist deferred to be run against a live dev session by the user. |
