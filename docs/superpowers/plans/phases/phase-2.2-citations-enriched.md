...
> **Status:** In progress (worktree `worktree-phase-2.2-citations-enriched`). **Spec:** §4 (amended 2026-04-17).
...
- [ ] **C. BYOK S2 key.** Extend `userApiKeys.providerType` enum with `"references"` (name per impl). Settings manager gains a Semantic Scholar row. Server helper `getUserS2Key(userId)` decrypts → `string|null`. Enrich route passes through to S2 client. Failing test first for helper + UI test for the row.
- [ ] **D. `CitationCard` overhaul.** `apps/web/src/components/reader/citation-card.tsx`:
...
- [ ] **E. Citations sidebar uses CitationCard compact + auto-enrichment.** `apps/web/src/components/reader/citations-sidebar.tsx`:
...
- [ ] **F. `/library/references` page uses CitationCard + remove.** `apps/web/src/app/(main)/library/references/page.tsx`:
  - Each row rendered as `<CitationCard variant="compact" />`.
