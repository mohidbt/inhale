...
| **Hotfix — local DB schema drift** | DONE | Chat was 500ing (`column "page_start" does not exist`) because local Postgres was out of sync with the schema. Root cause: Phase 1.1 added `pageStart`/`pageEnd`/`embedding` + ivfflat to `document_chunks` but no migration was ever generated or pushed. `drizzle/0002_enable_pgvector.sql` also existed but was never applied (not in journal, extension missing). Fix: built pgvector 0.8.2 from source against `postgresql@16` (brew's formula only ships pg17/pg18 binaries), then applied 4 raw SQL statements (`CREATE EXTENSION vector`, `ALTER TABLE ... ADD COLUMN page_start/page_end/embedding`, ivfflat `CREATE INDEX`). DB now matches schema. **Why E2E didn't catch it:** test 1 accepts `processingStatus: /^(ready\|failed)$/` so silent chunking failures passed; tests 2–4 mock the whole route via `page.route()`. Note: no git commit — all changes were DB-side. |
| **Migration rebaseline — drift debt** | NEXT | `drizzle/` has only `0000_goofy_spencer_smythe.sql` (pre-chunks); every table since (including `document_chunks`, `document_sections`, `user_highlights`, `user_comments`, agent tables) was created via ad-hoc `drizzle-kit push`, not tracked in any migration. A fresh `npm install` + `drizzle-kit migrate` against a clean DB will NOT reproduce the current state. **Todo:** (1) snapshot current schema into a new `0001_*.sql` + updated `_journal.json` + `drizzle/meta/0001_snapshot.json` so `drizzle-kit generate` works going forward (either by `drizzle-kit introspect` or by hand-rolling the missing DDL); (2) fold `drizzle/0002_enable_pgvector.sql` into a real migration entry (CREATE EXTENSION needs superuser, so keep it as a separate script or document that fact); (3) add a macOS setup note/README step for pgvector on pg16 (brew ships pg17/pg18 binaries only — need `make PG_CONFIG=...` build or switch to `pgvector/pgvector:pg16` Docker image the repo already specs in `docker-compose.yml`); (4) consider whether local dev should just use the Docker postgres instead of brew. |
| 2.0–3.3 | Pending | — |
| 4.0–4.4 | Pending | — |
